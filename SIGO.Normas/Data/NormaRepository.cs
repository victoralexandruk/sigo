...
namespace SIGO.Normas.Data
{
    public class NormaRepository : IRepository<Norma>
    {
        ...
        public IEnumerable<Norma> GetAll()
        {
            ...
            {
                return db.Query<Norma>("SELECT N.*, (SELECT COUNT(Id) FROM AcaoPlanejada AP WITH(NOLOCK) WHERE AP.CodigoNorma = N.Codigo) QtdAcoesPlanejadas FROM Norma N WITH(NOLOCK)");
            }
        }
